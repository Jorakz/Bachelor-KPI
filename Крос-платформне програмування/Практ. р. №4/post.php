...
function deal_with_table ( $mysqli, $query, $done )
{
  if ( $rslt = $mysqli->query($query) )
    return '`data` table successfully ' . $done;
  else
...
$fldNo
  ...
  $quan = sscanf($data['quan'], '%d')[0];
  $tick = sscanf($data['tick'], '%d')[0];
  if ( ! ($quan > 0)  )
    $err .= 'wrong field ' . "'quan'" . ' : the value less then zero : ' . "'" . $data['quan'] . "'" . '<br>';

  if ( ! ($tick > 0)  )
    $err .= 'wrong field  ' . "'tick'" . ' : the value less then zero : ' . "'" . $data['tick'] . "'" . '<br>';
  $mus = sscanf($data['mus'], '%d')[0];
  if ( ! ($mus == 0 || $mus == 1) )
    $err .= 'wrong field  ' . "'mus'" . ' : ' . 'the value must be zero or one' . ' : ' . "'" . $data['isdbt'] . "'" . '<br>';
...
$errs
  ...
  {
    $stmt->bind_param('dssssddd', $data[_id_], $data['nm'], $data['aut'], $data['the'], $data['per'], $data['quan'], $data['tick'], $data['mus']);

...
//******************************************************************************
	static private long skip ( InputStream IS, int Size ) throws IOException {
		long skipped = 0;
		for ( ; Size > 0; ){
			long _skipped = IS.skip(Size);
	    	if ( _skipped <= 0 )
				break;
	    	skipped += _skipped;
	    	Size -= _skipped;
...
//******************************************************************************
	static private int read (InputStream IS, byte Buff [ ], int Size)throws IOException{
		byte _buff [ ] = new byte [ Size ];
		int read = 0;
	  	for ( ; Size > 0; ){
	  		int _read = IS.read(_buff, 0, Size);
	      	if ( _read < 0 )
	      		break;
...
//******************************************************************************
	public DICOMImage ( String PathName, String FileName )throws FileNotFoundException, IOException, Exception{
		File initialFile = new File(PathName + "/" + FileName);
    	InputStream fis = new FileInputStream(initialFile);

		// InputStream fis = (new URL("file:///" + PathName + "/" + FileName)).openStream();
		...
		int read = read(fis, buf4, 4);
  		if (! new String(buf4).equals("DICM")){
  	  		fis.close();
  	  		throw new Exception("Файлів DICOM не знайдено");
  		}
		...
		boolean expl = true;
  		...
  		for ( ; ; ){
  			...
  			String text = "";
  	  		...
  	  		if ( expl || tag2[0] == 0x0002 ){
  	  			...
  	  			dt = new String(buf4, 0, 2);
					  ...
					  dt.equals("UN")){
  	  		  	  ...
  	  		  	  len4 = (int) (((buf4[3] & 0xFF) << 24) | ((buf4[2] & 0xFF) << 16) | ((buf4[1] & 0xFF) << 8) | (buf4[0] & 0xFF));
  	  		  	}
  	  		  	else{
  	  		  	  read = read(fis, buf4, 2);
  	  		  	  len4 = (int) (((buf4[1] & 0xFF) << 8) | (buf4[0] & 0xFF));
  	  		  	}
  	  		} else {
  	  		// implicit datatype
  	    		dt = "  ";
  	  		...
  	  		} else if ( tag2[0] == 0x0008 && (tag2[1] == 0x1110 || 
				tag2[1] == 0x1111 || tag2[1] == 0x1115 || tag2[1] == 0x1120)){
  	  			// 'Referenced Sequences'
  	  			byte buff [ ] = new byte [ 12 ];
  	    		read = read(fis, buff, 8);
  	  		} else if ( tag2[0] == 0x0028 && (tag2[1] == 0x0010 || tag2[1] == 0x0011)){
  	  			// width || height
  	  			byte ch [ ] = new byte [ len4 ];
  	    		read = read(fis, ch, len4);
  	    		if ( tag2[1] == 0x0010 )
  	      			Ht = (int) (((ch[1] & 0xFF) << 8) | (ch[0] & 0xFF));
  	    		else
  	      			Wd = (int) (((ch[1] & 0xFF) << 8) | (ch[0] & 0xFF));
  	    		...
  	    		for (int pxlNo = 0; pxlNo < bytes; pxlNo += 2){
					int val = (((int)data[pxlNo + 1] & 0xFF) << 8) | ((int)data[pxlNo] & 0xFF);
  	      			if ( val > max ) max = val;
  	      			if ( val < min ) min = val;
  	    		}
  	    		...
  	    		for (int pxlNo = 0, yy = 0; yy < Ht; yy++)
					for (int xx = 0; xx < Wd; xx++, pxlNo += 2){
  	    		  		int val = (max != 0 ? ( (((int)data[pxlNo + 1] & 0xFF) << 8) | ((int)data[pxlNo] & 0xFF) ) * 255 / max : 0);
  	  		...
  	  		if ( (tag2[0] == 0x0008 && (tag2[1] == 0x0032 || tag2[1] == 0x0033)) || (tag2[0] == 0x0018 && tag2[1] == 0x1060)){
  	    		...
  	    		if ( tag2[0] == 0x0018 )
  	    			TrigTime = time;
  	    		  	...
  	    		  	ContTime = time;
  	  		} else if (tag2[0] == 0x0020 && (tag2[1] == 0x0011 || 
				tag2[1] == 0x0012 || tag2[1] == 0x0013 || tag2[1] == 0x1041)){
  	  			// 'Series Number' || 'Acquisition Number' || 'Instance Number' || 'Slice Location'
  	    		switch ( tag2[1] )
  	    		{
  	    		case 0x0011 : SeriesNumber  = Integer.parseInt(text); break;
  	    		case 0x0012 : AcquNumber    = Integer.parseInt(text); break;
  	    		case 0x0013 : InstNumber    = Integer.parseInt(text); break;
  	    		case 0x1041 : SliceLocation = Double.parseDouble(text); break;
  	    		}
  	  		}
	...
	@Override
	public int compareTo(DICOMImage i) {
	  	  return Comparator.comparing((DICOMImage i2) ->
	  	                  i2.SliceLocation, Comparator.reverseOrder())
	            .thenComparingInt((DICOMImage i2) ->
	                    i2.SeriesNumber)
	            .thenComparingInt((DICOMImage i2) ->
	                    i2.AcquNumber)
	            .thenComparingInt((DICOMImage i2) ->
